...
func (c *Cloud) providerIDV2(ctx context.Context, nodeMD NodeMetadata) string {
	...
	if isProviderPowerVS(c.Config.Prov) {
		if c.Config.Prov.PowerVSRegion != "" && c.Config.Prov.PowerVSZone != "" {
	...
	if isProviderPowerVSWithMultiWorkspace(c.Config.Prov) {
		return fmt.Sprintf("ibmpowervs://%s/%s/%s/%s", nodeMD.Region, nodeMD.FailureDomain, nodeMD.PowerVSWorkspaceID, nodeMD.WorkerID)
	}
